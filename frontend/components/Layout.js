    ...
    <div className={`flex flex-col h-full ${mobile ? "w-full" : "w-64"}`}>
      ...
      <div className="p-6 border-b hairline">
        <div className="flex items-center gap-3">
          ...
          />
          <div>
            <h1 className="portal-heading text-ink text-sm leading-tight">
              Smart Business
            </h1>
      ...
      <div className="px-4 py-4 border-b hairline">
        <div className="flex items-center gap-3 p-3 rounded-xl bg-ground-secondary/30">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="portal-label text-muted truncate">
          ...
          <>
            ...
            {userNavItems.map(({ href, label, icon: Icon }) => (
              <Link
                ...
                className={`portal-nav-link flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 font-medium ${router.pathname === href ? "bg-amber/10 text-amber" : "text-ink-secondary hover:text-ink hover:bg-ground-secondary/50"}`}
              >
      ...
      <div className="p-4 border-t hairline">
        <button
          ...
          className="portal-nav-link w-full flex items-center gap-3 px-4 py-3 rounded-xl text-red-400 hover:text-red-300 hover:bg-red-500/10 transition-all"
        >
          <LogOut size={18} />
        ...
        <div className="lg:hidden fixed inset-0 z-40 flex">
          ...
          <aside className="relative z-50 w-72 bg-ground-secondary border-r hairline shadow-2xl flex flex-col">
            <div className="absolute top-4 right-4">
              <button
                onClick={() => setSidebarOpen(false)}
                className="p-2 rounded-lg hover:bg-ground"
              >
                <X size={18} />
              </button>
            </div>
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <button
                ...
                className="lg:hidden p-2 rounded-lg hover:bg-ground-secondary text-ink-secondary"
              >
                <Menu size={20} />
              </button>
              <div>
            ...
            <div className="flex items-center gap-3">
              <div className="hidden sm:flex items-center gap-2 bg-ground-secondary/80 border hairline rounded-xl px-3 py-2 w-56">
                ...
                <input
                  ...
                  className="bg-transparent text-sm text-ink-secondary placeholder-muted outline-none w-full"
                />
              </div>
                        ...
                        {notifications.map((notification) => (
                          ...
                          >
                            <div className="flex items-start gap-3">
                              <div
                                className={`w-2 h-2 rounded-full mt-2 ${notification.read ? "bg-muted" : "bg-amber"}`}
                              />
                              <div className="flex-1 min-w-0">
                                <p className="font-medium text-ink text-sm">
                                  {notification.title}
                                </p>
                                <p className="portal-label text-muted text-xs mt-1 line-clamp-2">
                ...
                {showProfileMenu && (
                  <div className="absolute right-0 top-full mt-2 bg-ground-secondary border hairline rounded-xl shadow-xl py-2 min-w-[200px] z-50">
                    ...
                    <Link
                      ...
                      className="flex items-center gap-3 px-4 py-2.5 text-sm text-ink-secondary hover:bg-ground transition-colors"
                    >
                      <User size={16} />
                    ...
                    <button
                      onClick={handleLogout}
                      className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-red-400 hover:bg-ground hover:text-red-300 transition-colors"
                    >
                      <LogOut size={16} />
