...
import { ShoppingBag, ArrowRight, Star, Truck, Shield, RotateCcw, Heart, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function StorefrontHomePage() {
  ...
  const features = [
    ...
    { icon: Star, title: 'Top Quality', description: 'Premium products guaranteed' },
  ];

  const testimonials = [
    { name: 'Sarah Johnson', role: 'Verified Buyer', content: 'Amazing quality and fast shipping! Will definitely order again.', rating: 5 },
    { name: 'Michael Chen', role: 'Verified Buyer', content: 'Best customer service I\'ve experienced. Highly recommended!', rating: 5 },
    { name: 'Emily Davis', role: 'Verified Buyer', content: 'Products exceeded my expectations. Great value for money.', rating: 5 },
  ];

  const nextSlide = () => setCurrentSlide((prev) => (prev + 1) % (featuredProducts.length || 1));
  const prevSlide = () => setCurrentSlide((prev) => (prev - 1 + (featuredProducts.length || 1)) % (featuredProducts.length || 1));
  ...
  return (
    ...
    >
      {/* Hero Section */}
      <section className="relative py-16 md:py-24 lg:py-32" style={{ 
        ...
        <div className={`${containerWidth} mx-auto px-4 sm:px-6 lg:px-8 relative`}>
          {heroLayout === 'split' ? (
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <div>
                <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight" style={{ color: textColor, fontFamily: storeSettings.heading_font_family }}>
                  {storeSettings.hero_title || 'Welcome to Our Store'}
                </h1>
                <p className="text-lg md:text-xl mb-8 max-w-xl" style={{ color: textSecondaryColor }}>
                  {storeSettings.hero_subtitle || 'Discover amazing products at unbeatable prices. Quality you can trust.'}
                </p>
                {storeSettings.hero_button_text && (
                  <a 
                    href={storeSettings.hero_button_link || `/storefront/${userId}/products`}
            ...
            <div className="relative rounded-2xl overflow-hidden" style={{ borderRadius: storeSettings.border_radius }}>
              {storeSettings.hero_image_url && (
                <img 
                  src={storeSettings.hero_image_url} 
                  alt={storeSettings.store_name} 
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-black/60 via-black/30 to-transparent flex items-center">
                <div className={`${containerWidth} mx-auto px-4 sm:px-6 lg:px-8`}>
                  <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight text-white" style={{ fontFamily: storeSettings.heading_font_family }}>
                    {storeSettings.hero_title || 'Welcome to Our Store'}
                  </h1>
                  <p className="text-lg md:text-xl mb-8 max-w-xl text-white/90" style={{ fontFamily: storeSettings.font_family }}>
                    {storeSettings.hero_subtitle || 'Discover amazing products at unbeatable prices. Quality you can trust.'}
                  </p>
                  {storeSettings.hero_button_text && (
                    <a 
            ...
            <div className="text-center max-w-4xl mx-auto">
              ...
              {storeSettings.hero_image_url && (
                <div className="mt-12 rounded-2xl overflow-hidden shadow-2xl" style={{ borderRadius: storeSettings.border_radius }}>
                  <img 
                    src={storeSettings.hero_image_url} 
                    alt={storeSettings.store_name} 
                    className="w-full"
      ...
      {/* Features Bar */}
      <section className="py-8 border-y" style={{ borderColor: storeSettings.border_color, backgroundColor: 'rgba(0,0,0,0.02)' }}>
        <div className={`${containerWidth} mx-auto px-4 sm:px-6 lg:px-8`}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {features.map((feature, index) => (
              <div key={index} className="flex items-center gap-4 p-4 rounded-xl hover:bg-white/50 transition-colors" style={{ borderRadius: storeSettings.border_radius }}>
                ...
                <div>
                  <h3 className="font-semibold" style={{ color: textColor, fontFamily: storeSettings.heading_font_family }}>{feature.title}</h3>
        ...
        <section className="py-16">
          <div className={`${containerWidth} mx-auto px-4 sm:px-6 lg:px-8`}>
            <div className="flex items-center justify-between mb-8">
              <div>
                <h2 className="section-title" style={{ fontFamily: storeSettings.heading_font_family }}>
                  {storeSettings.featured_products_title || 'Featured Products'}
                </h2>
                <p className="section-subtitle" style={{ fontFamily: storeSettings.font_family }}>
            ...
            <div className="text-center mt-8 sm:hidden">
              <Link href={`/storefront/${userId}/products`} className="btn-primary inline-flex items-center gap-2">
        ...
        <section className="py-16" style={{ backgroundColor: 'rgba(0,0,0,0.02)' }}>
          <div className={`${containerWidth} mx-auto px-4 sm:px-6 lg:px-8`}>
            ...
            <div className="text-center mt-8">
              <Link href={`/storefront/${userId}/categories`} className="btn-outline inline-flex items-center gap-2">
      ...
      {storeSettings.show_testimonials && (
        <section className="py-16">
          <div className={`${containerWidth} mx-auto px-4 sm:px-6 lg:px-8`}>
            <div className="text-center mb-12">
              <h2 className="section-title" style={{ fontFamily: storeSettings.heading_font_family }}>What Our Customers Say</h2>
              <p className="section-subtitle mt-2" style={{ fontFamily: storeSettings.font_family }}>Real reviews from real customers</p>
            </div>

            <div className="grid md:grid-cols-3 gap-6">
              {testimonials.map((testimonial, index) => (
                <div key={index} className="card p-6 text-center" style={{ borderRadius: storeSettings.border_radius }}>
                  <div className="flex items-center justify-center gap-1 mb-4 text-yellow-400">
                    {[...Array(testimonial.rating)].map((_, i) => <Star key={i} size={20} fill className="text-yellow-400" />)}
                  </div>
                  <p className="mb-6" style={{ color: textSecondaryColor, fontFamily: storeSettings.font_family }}>
                    "{testimonial.content}"
                  </p>
                  <div>
                    <p className="font-semibold" style={{ color: textColor }}>{testimonial.name}</p>
                    <p className="text-sm" style={{ color: textSecondaryColor }}>{testimonial.role}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {storeSettings.show_newsletter && (
        <section className="py-16" style={{ backgroundColor: secondaryColor }}>
          <div className={`${containerWidth} mx-auto px-4 sm:px-6 lg:px-8 text-center`}>
            ...
            <p className="text-white/80 mb-8 max-w-2xl mx-auto" style={{ fontFamily: storeSettings.font_family }}>
              {storeSettings.newsletter_subtitle || 'Get updates on new products, special offers, and more.'}
            </p>
            <form className="max-w-md mx-auto flex gap-2" onSubmit={(e) => { e.preventDefault(); alert('Thanks for subscribing!'); }}>
              <input 
                type="email" 
                ...
                required
              />
              <button type="submit" className="btn-accent px-6">Subscribe</button>
            </form>
          </div>
        </section>
      )}
